...
export function createRealParticipantPhysicalTestPort(): RealParticipantPhysicalTestPort {
  return {
    ...
    readTrainerLessonPlan(classModuleId: string | null): Promise<UiActionResult<TrainerLessonPlanDto | null>> {
      return guard(() => adapterReadTrainerLessonPlan(classModuleId));
    },
    ...
    updateStudent(input: UpdateStudentInput): Promise<UiActionResult<UpdateStudentOutcomeDto>> {
      return guard(() => adapterUpdateStudent(input));
    },

    /** `P2-14` — screen `22`. */
    withdrawStudent(studentId: string): Promise<UiActionResult<WithdrawStudentOutcomeDto>> {
      return guard(() => adapterWithdrawStudent(studentId));
    },
