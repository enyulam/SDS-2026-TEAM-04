...
|---|---|
| Screen ID | `25` |
| Screen name | Management Schedule |
| Role | Management |
| Folder | `25-management-schedule` |
| Canonical route | `/management/schedule` |
...
| Figma node | `506:59` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=506-59&m=dev |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Schedule/`** — the ratified Final MVP visual source for this screen. Frame `Management - Schedule.png`, HTML render `Management - Schedule.html`, pack notes `Management - Schedule.md`.
...
Management calendar over class sessions, as a projection of class-session records.
...
Route, layout and components to node `506:59` once a management calendar projection exists.
...
Do not create a separate duplicated event or calendar table (A-016).
...
**Missing** â€” no management calendar projection.

### Governance

`Post-48-hour final-MVP scope` (A-044). The calendar is a **projection** of class-session records and their assignments; no duplicated event table may be created (A-016, A-047).
