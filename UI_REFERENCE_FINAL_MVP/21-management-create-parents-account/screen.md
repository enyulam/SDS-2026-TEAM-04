...
|---|---|
| Screen ID | `21` |
| Screen name | Management Create Parent Account |
| Role | Management |
| Folder | `21-management-create-parents-account` |
| Canonical route | `/management/students/create-parent-account` |
...
| Figma node | `709:2` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=709-2&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Create Parents Account/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Create Parents Account/Management - Create Parents Account.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Create Parents Account/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Create Parents Account/`** — the ratified Final MVP visual source for this screen. Frame `Management - Create Parents Account.png`, HTML render `Management - Create Parents Account.html`, pack notes `Management - Create Parents Account.md`.
...
Management initiates a parent invitation. The recipient verifies the account and establishes their own login credentials.
...
None until the exact field inventory is ratified. Visual reconstruction may be prepared against node `709:2` without inventing fields or persistence.
...
Never store, display, email or log a plaintext generated password (A-020). No application table may hold an authentication secret (A-027). Do not invent a parent-profile field.
...
**Missing** â€” no invitation write path; `invitations` holds zero policies and zero client privileges.

### Governance

`Post-48-hour final-MVP scope` (A-044) **+ `Governance decision missing`** â€” the exact parent-profile field inventory is UNRESOLVED. **No plaintext password may ever appear** (A-020, A-027).
