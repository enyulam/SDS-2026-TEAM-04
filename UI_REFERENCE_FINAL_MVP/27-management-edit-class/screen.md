...
|---|---|
| Screen ID | `27` |
| Screen name | Management Edit Class |
| Role | Management |
| Folder | `27-management-edit-class` |
...
| Figma node | `545:9` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=545-9&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Class/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Class/Management - Edit Class.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Class/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Class/`** — the ratified Final MVP visual source for this screen. Frame `Management - Edit Class.png`, HTML render `Management - Edit Class.html`, pack notes `Management - Edit Class.md`.
...
Management edits an existing Class Module under its Class Grade.
...
Route, layout and components to node `545:9` once a class-module edit write path exists.
...
Do not introduce a hidden `classes` entity (A-016). Do not create a fourth Class Grade (A-026, A-054).
...
**Missing** â€” no class-module edit write path.

### Governance

`Post-48-hour final-MVP scope` (A-044). "Class type" presentation maps to the governed Class Module concept (A-047).
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Class/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
