...
| Screenshot status | Validated — ready for implementation. **Current visual reference: `UI_REFERENCE_FINAL_MVP/reference/Parent - Report/`**; the pack-local `reference.png` is a frozen SHA-identical duplicate of it. |
...
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Parent - Report/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Parent - Report/`** — the ratified Final MVP visual source for this screen. Frame `Parent - Report.png`, HTML render `Parent - Report.html`, pack notes `Parent - Report.md`.
