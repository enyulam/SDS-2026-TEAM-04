...
|---|---|
| Screen ID | `13` |
| Screen name | Management Class Overview |
| Role | Management |
| Folder | `13-management-class-overview` |
...
| Figma node | `689:66` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=689-66&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Class Overview/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Class Overview/Management - Class Overview.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Class Overview/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Class Overview/`** — the ratified Final MVP visual source for this screen. Frame `Management - Class Overview.png`, HTML render `Management - Class Overview.html`, pack notes `Management - Class Overview.md`.
...
Management overview of one Class Module, including per-row status gating governed by `CLAUDE.md` Â§6 and A-038.
...
Route, layout and components to node `689:66` once a class-overview projection exists.
...
Do not infer per-row status gating from the frame â€” it is ratified elsewhere. Do not introduce a hidden `classes` entity (A-016).
...
**Missing** â€” no class-overview projection; per-row status gating is ratified but unbuilt.

### Governance

`Post-48-hour final-MVP scope` (A-044). Per-row status gating is fully ratified (`CLAUDE.md` Â§6, A-038) and must be built to that rule, **not inferred from the frame**. "Class type" maps to Class Module (A-047).
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Class Overview/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
