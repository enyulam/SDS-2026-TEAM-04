...
|---|---|
| Screen ID | `28` |
| Screen name | Management Term Report |
| Role | Management |
| Folder | `28-management-term-report` |
| Canonical route | `/management/students/[studentId]/term-report` |
| Current implemented route | â€” (no implemented route) |
| Route-compatibility treatment | Not applicable â€” **separately governed** |
| Figma file key | `sSY1TYw3jyVlZDy8V2Mu7g` |
| Figma file name | `SDS-dashboard` |
| Figma node | `836:2` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=836-2&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Term Report/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Term Report/Management - Term Report.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Term Report/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Term Report/`** — the ratified Final MVP visual source for this screen. Frame `Management - Term Report.png`, HTML render `Management - Term Report.html`, pack notes `Management - Term Report.md`.
...
**Visual target only** until its AI derivation, source-data model, approval, submission and Parent-visibility model are separately governed.
...
Visual reference only. No route, generator, schema or write path may be created under this pack.
...
Do not build term-report generation. Do not create a term-report table, RPC or generator. Do not treat the restated 4-level to 3-level map as ratified â€” it keeps exactly the status it already had: a proposed, trainer-overridable default used only when term generation is built.
...
**Missing** â€” no term-report instrument, generator, schema or governance.

### Governance

**`Governance decision missing`** â€” term-report **generation is explicitly out of MVP scope** (`CLAUDE.md` Â§5, Â§8; spec Â§28). Separately governed before implementation. No screen in the inventory authorizes it (A-048).
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Term Report/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
...
Management Term Report: **visual target only** until its AI derivation, source-data model, approval, submission and Parent-visibility model are separately governed.
