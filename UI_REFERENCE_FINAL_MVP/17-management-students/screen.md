...
|---|---|
| Screen ID | `17` |
| Screen name | Management Students |
| Role | Management |
| Folder | `17-management-students` |
| Canonical route | `/management/students` |
...
| Figma node | `510:9` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=510-9&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Students/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Students/Management - Students.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Students/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Students/`** — the ratified Final MVP visual source for this screen. Frame `Management - Students.png`, HTML render `Management - Students.html`, pack notes `Management - Students.md`.
...
Management list over the centre's students.
...
Route, layout and components to node `510:9` once a management student-list projection exists.
...
Do not query student tables directly from the client (ADR-3). Do not expose students outside the authenticated centre membership.
...
**Missing** â€” no management student-list projection.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Students/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
