...
|---|---|
| Screen ID | `18` |
| Screen name | Management Student Profile |
| Role | Management |
| Folder | `18-management-student-profile` |
| Canonical route | `/management/students/[studentId]` |
...
| Figma node | `649:9` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=649-9&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Student Profile/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Student Profile/Management - Student Profile.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Student Profile/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Student Profile/`** — the ratified Final MVP visual source for this screen. Frame `Management - Student Profile.png`, HTML render `Management - Student Profile.html`, pack notes `Management - Student Profile.md`.
...
Management profile view of one student.
...
Route, layout and components to node `649:9` once a student-profile projection exists.
...
Do not add an account or login linkage to a student (A-025). Do not invent profile fields â€” the exact field inventory is an open governance decision.
...
**Missing** â€” no student-profile projection.

### Governance

`Post-48-hour final-MVP scope` (A-044). `students` have no `auth_user_id` and no account linkage, so a student login is structurally impossible (A-025).

---

## 8. Competency vocabulary

**Not established.** Whether this screen surfaces competency ratings is not ratified. Do not infer a rating surface from the frame.

If a later governance decision gives this screen a rating surface, Beginning / Developing / Mastering / Mastered is ratified by Amendment 006, and code implementation remains pending Backend V2 and Frontend V3.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Student Profile/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
...
Management Student Report: **Amendment 004's wording-only edit boundary remains controlling** for every Management surface that reaches report content.
