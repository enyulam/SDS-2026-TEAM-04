...
|---|---|
| Screen ID | `11` |
| Screen name | Management Dashboard |
| Role | Management |
| Folder | `11-management-dashboard` |
| Canonical route | `/management/dashboard` |
| Current implemented route | /management |
| Route-compatibility treatment | Preserve existing route as redirect â€” `/management` redirects to `/management/dashboard` |
| Figma file key | `sSY1TYw3jyVlZDy8V2Mu7g` |
| Figma file name | `SDS-dashboard` |
| Figma node | `397:2` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=397-2&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Dashboard/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Dashboard/Management - Dashboard.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Dashboard/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Dashboard/`** — the ratified Final MVP visual source for this screen. Frame `Management - Dashboard.png`, HTML render `Management - Dashboard.html`, pack notes `Management - Dashboard.md`.
...
Management landing surface for the final MVP.
...
The existing `/management` landing surface and its queue reads.

### Allowed frontend expansion

Build the canonical `/management/dashboard` route to node `397:2`; preserve `/management` as a redirect.
...
Do not invent centre-wide aggregates that no delivered projection supplies.
...
Centre-scoped summary projection â€” **not delivered**. The current landing surface reads the queue projections.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Dashboard/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
