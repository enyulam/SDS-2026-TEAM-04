...
|---|---|
| Screen ID | `14` |
| Screen name | Management Lesson Plan Management |
| Role | Management |
| Folder | `14-management-lesson-plan-management` |
| Canonical route | `/management/classes/[classModuleId]/lesson-plans` |
...
| Figma node | `760:2` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=760-2&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Lesson Plan Management/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Lesson Plan Management/Management - Lesson Plan Management.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Lesson Plan Management/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Lesson Plan Management/`** — the ratified Final MVP visual source for this screen. Frame `Management - Lesson Plan Management.png`, HTML render `Management - Lesson Plan Management.html`, pack notes `Management - Lesson Plan Management.md`.
...
Management administration of lesson plans for a Class Module. Blocked on unratified governance, not merely on missing code.
...
None until lesson-plan governance is ratified. Visual reconstruction may be prepared against node `760:2` without inventing persistence.
...
Do not create a lesson-plan table, column, enum or RPC. Do not infer a lesson-plan schema from the frame (A-022).
...
**Missing** â€” no lesson-plan schema, RPC or governance exists in the ratified 26-table / 12-enum census.

### Governance

`Post-48-hour final-MVP scope` (A-044) **+ `Backend dependency missing`**. Lesson-plan governance is not ratified.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Lesson Plan Management/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
