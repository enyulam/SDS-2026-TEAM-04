...
| Figma node | `459:13` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=459-13&m=dev |
...
| 48-hour core membership | **Yes** - core, flow order 7 |
| Physical-test flow order | 7 |
...
| Screenshot status | Validated — ready for implementation. **Current visual reference: `UI_REFERENCE_FINAL_MVP/reference/Auth 02 - Mangement - Login/`**; the pack-local `reference.png` is a frozen SHA-identical duplicate of it. |
...
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Auth 02 - Mangement - Login/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Auth 02 - Mangement - Login/`** — the ratified Final MVP visual source for this screen. Frame `Auth 02 - Mangement - Login.png`, HTML render `Auth 02 - Mangement - Login.html`, pack notes `Auth 02 - Mangement - Login.md`.
...
Management authenticates with a real Supabase Auth identity. The `?role=management` query selects presentation only.
...
Visual reconstruction of the login shell to node `459:13`; role-specific presentation variants; loading, validation, error and disabled states.
...
Do not grant Management authority from the query parameter. Do not present the named management account as a shared login. Do not change redirect policy independently.
...
A-046, A-020, A-027, A-015 (the single named management account is one named account, never a shared login), ADR-4.
