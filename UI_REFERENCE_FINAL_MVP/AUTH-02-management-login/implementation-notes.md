# AUTH-02 - Management Login - implementation notes
...
## FRONTEND RECONSTRUCTION F10 — AUTH-02 (459:13)

| Field | Value |
|---|---|
| Timestamp (Asia/Singapore) | 2026-08-06 |
| Checkpoint | `FRONTEND RECONSTRUCTION F10` |
| Branch | `feat/48h-frontend` |
| Starting commit | `b72752a88ed84144a135d19b64aea2c1658ceb95` |
| Ending commit | `52d26d08f6ae3439b9bf46c4561c17ed1f4acfb2` |
| Canonical route | `/login?role=management` — satisfied, no mismatch |
| Reference SHA-256 | `fcc3db9377a1b1175984bc90732c588e58bd05269d767af2ee69ed8d42668483` — **verified identical to the validated pack**, 95,584 bytes, 1440 x 1024 |
...
Measured column geometry (400 px wide starting at x = 520 at the 1440 px reference viewport, matching the frozen frame exactly) and reference vertical ordering of the seven landmarks; Management-specific pre-authentication non-disclosure of `queue`, `pending review`, `final review`, `wording`, `correction`, `return to trainer`, `publish`, `submit`; shared/demo credential assertion (A-015, A-046).
...
- **Destination:** expected physical-test destination `/management/reports`. The expected physical-test destination `/management/reports` EXISTS and is already canonical with no route mismatch. Wiring the real post-authentication destination is F16.
...
Evidence (outside Git): `UI_REFERENCE_FINAL_MVP\_checkpoint-evidence\F10\`
...
Existing route audited:        /login?role=management
