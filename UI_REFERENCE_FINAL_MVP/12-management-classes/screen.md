...
|---|---|
| Screen ID | `12` |
| Screen name | Management Classes |
| Role | Management |
| Folder | `12-management-classes` |
...
| Figma node | `442:9` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=442-9&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Classes/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Classes/Management - Classes.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Classes/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Classes/`** — the ratified Final MVP visual source for this screen. Frame `Management - Classes.png`, HTML render `Management - Classes.html`, pack notes `Management - Classes.md`.
...
Management list over Class Modules under their Class Grades.
...
Route, layout and components to node `442:9` once a class-module list projection exists.
...
Do not introduce a hidden `classes` entity between Class Grade and Class Module (A-016). Do not create a fourth Class Grade (A-026, A-054).
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Classes/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
