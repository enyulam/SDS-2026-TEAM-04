...
|---|---|
| Screen ID | `09` |
| Screen name | Trainer Reports |
| Role | Trainer |
| Folder | `09-trainer-reports` |
| Canonical route | `/trainer/reports` |
| Current implemented route | /trainer/reports (currently serves `?status=needs_edit` only) |
| Route-compatibility treatment | No route mismatch â€” canonical route satisfied; screen content incomplete. `?status=needs_edit` preserved as a compatibility alias |
| Figma file key | `sSY1TYw3jyVlZDy8V2Mu7g` |
| Figma file name | `SDS-dashboard` |
| Figma node | `783:59` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=783-59&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Trainer - Reports/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Trainer - Reports/Trainer - Reports.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Trainer - Reports/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Trainer - Reports/`** — the ratified Final MVP visual source for this screen. Frame `Trainer - Reports.png`, HTML render `Trainer - Reports.html`, pack notes `Trainer - Reports.md`.
...
Trainer list of reports. The canonical route exists but currently serves only the returned-correction queue.
...
The canonical `/trainer/reports` route, the R-4 returned-correction queue and the `?status=needs_edit` query variant.

### Allowed frontend expansion

Visual reconstruction to node `783:59` and the general reports list once its projection exists.
...
Do not invent a general reports list projection. Do not expose reports outside the trainer's live assignments.
...
Returned-correction queue (R-4) â€” **delivered**. A general trainer reports list is **not** delivered.

### Governance

`Post-48-hour final-MVP scope` (A-044).
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Trainer - Reports/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
