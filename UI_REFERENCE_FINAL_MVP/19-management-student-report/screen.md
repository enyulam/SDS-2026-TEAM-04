...
|---|---|
| Screen ID | `19` |
| Screen name | Management Student Report |
| Role | Management |
| Folder | `19-management-student-report` |
| Canonical route | `/management/students/[studentId]/reports/[reportId]` |
| Current implemented route | /management/reports/[reportId]/review, /management/reports/[reportId]/edit |
| Route-compatibility treatment | Replace after integration; pinned paths preserved as redirects. The canonical route adds a `[studentId]` segment the implemented route resolves server-side |
| Figma file key | `sSY1TYw3jyVlZDy8V2Mu7g` |
| Figma file name | `SDS-dashboard` |
| Figma node | `648:330` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=648-330&m=dev |
...
| 48-hour core membership | **Yes** - core, flow order 9 |
| Physical-test flow order | 9 |
...
| Screenshot status | Validated — ready for implementation. **Current visual reference: `UI_REFERENCE_FINAL_MVP/reference/Management - Student Report/`**; the pack-local `reference.png` is a frozen SHA-identical duplicate of it. |
| Native dimensions | **1440 × 1330 px** (aspect 144:133) - measured from the frozen file |
...
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Student Report/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Student Report/`** — the ratified Final MVP visual source for this screen. Frame `Management - Student Report.png`, HTML render `Management - Student Report.html`, pack notes `Management - Student Report.md`.
...
Management may edit parent-facing wording only, return assessment-level concerns to Trainer, or perform final Approve & Submit.
...
The pinned `/review` and `/edit` sub-routes, the wording-only edit boundary, the return-to-trainer path, the final Approve & Submit transition and dual approval provenance.

### Allowed frontend expansion

Visual reconstruction to node `648:330`; add the canonical `[studentId]`/`[reportId]` route; review, wording-edit, return and submit states.
...
Do not let Management modify a rating, observation, attendance record, evidence item or trainer note. Do not widen the wording-edit boundary beyond the four parent-facing panels (A-034). Do not publish anything other than through Approve & Submit.
...
`report_get_management_review` (RPC-15), `report_get_canonical` (RPC-13), `report_management_edit_wording` (RPC-9), `report_management_return_to_trainer` (RPC-10), `report_management_approve_and_submit` (RPC-11) â€” **delivered**.

### Governance

Amendment 004 A-034/A-038: Management may edit **only** the four parent-facing wording panels, may **return** a report for a rating, observation or derived assessment-fact issue, and performs the final Approve & Submit. Management **never** modifies ratings, observations, attendance, evidence, trainer notes or any underlying assessment fact. Approve & Submit is the only action that makes a report parent-visible (A-033).
...
- [ ] Only the four parent-facing wording panels are editable.
- [ ] Return-to-trainer is available for a rating, observation or derived assessment-fact concern.
- [ ] Approve and Submit is the only action that makes a report parent-visible.
...
Management Student Report: **Amendment 004's wording-only edit boundary remains controlling.** Management may edit only the four parent-facing wording panels, and never a rating, observation, attendance record, evidence item or trainer note.
