...
|---|---|
| Screen ID | `03` |
| Screen name | Trainer Lesson Plan |
| Role | Trainer |
| Folder | `03-trainer-lesson-plan` |
| Canonical route | `/trainer/my-classes/lesson-plan` |
...
| Figma node | `773:2` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=773-2&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Trainer - Lesson Plan/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Trainer - Lesson Plan/Trainer - Lesson Plan.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Trainer - Lesson Plan/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Trainer - Lesson Plan/`** — the ratified Final MVP visual source for this screen. Frame `Trainer - Lesson Plan.png`, HTML render `Trainer - Lesson Plan.html`, pack notes `Trainer - Lesson Plan.md`.
...
Trainer view of the lesson plan for a Class Module. Blocked on unratified governance, not merely on missing code.
...
None until lesson-plan governance is ratified. Visual reconstruction may be prepared against node `773:2` without inventing persistence.
...
**Missing** â€” no lesson-plan table, enum or RPC exists in the ratified 26-table / 12-enum census. Lesson plans have no schema, no RPC and no ratified governance.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Trainer - Lesson Plan/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
