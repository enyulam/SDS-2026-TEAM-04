...
|---|---|
| Screen ID | `16` |
| Screen name | Management Class Statistics |
| Role | Management |
| Folder | `16-management-class-statistics` |
| Canonical route | `/management/classes/[classModuleId]/class-statistics` |
...
| Figma node | `445:9` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=445-9&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Class Statistics/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Class Statistics/Management - Class Statistics.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Class Statistics/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Class Statistics/`** — the ratified Final MVP visual source for this screen. Frame `Management - Class Statistics.png`, HTML render `Management - Class Statistics.html`, pack notes `Management - Class Statistics.md`.
...
Management class-level statistics over a Class Module, including the governed "Management Insight" content.
...
Route, layout and components to node `445:9` once a class-statistics projection exists.
...
Do not author "Management Insight" copy freehand â€” it follows the exact deterministic template in `CLAUDE.md` Â§6. Do not compute statistics client-side.
...
**Missing** â€” no class-statistics projection.

### Governance

`Post-48-hour final-MVP scope` (A-044) **+ `Backend dependency missing`**. "Management Insight" content is governed by `CLAUDE.md` Â§6's exact deterministic template.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Class Statistics/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
