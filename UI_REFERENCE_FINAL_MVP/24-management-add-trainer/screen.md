...
|---|---|
| Screen ID | `24` |
| Screen name | Management Add Trainer |
| Role | Management |
| Folder | `24-management-add-trainer` |
| Canonical route | `/management/trainers/add` |
...
| Figma node | `544:292` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=544-292&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Add Trainer/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Add Trainer/Management - Add Trainer.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Add Trainer/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Add Trainer/`** — the ratified Final MVP visual source for this screen. Frame `Management - Add Trainer.png`, HTML render `Management - Add Trainer.html`, pack notes `Management - Add Trainer.md`.
...
Management initiates a trainer invitation. The recipient verifies the account and establishes their own login credentials.
...
None until the exact field inventory is ratified. Visual reconstruction may be prepared against node `544:292` without inventing fields or persistence.
...
Never store, display, email or log a plaintext generated password (A-020, A-027). Do not invent a trainer-profile field. Do not put a role on an identity row (A-020).
...
**Missing** â€” no trainer-creation/invitation write path.

### Governance

`Post-48-hour final-MVP scope` (A-044) **+ `Governance decision missing`** â€” the exact trainer-profile field inventory is UNRESOLVED.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Add Trainer/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
