# 24 - Management Add Trainer - implementation notes
...
- **GC-11 — A "Role" dropdown on the trainer identity form offering "Assistant Trainer". GROUNDS (re-attributed 2026-08-08): A-020 and A-025 remain FULLY ACTIVE but were misapplied here — they prohibit a role COLUMN on accounts, not a selector on a creation form. The ACTUAL grounds are narrower and stronger: "Assistant Trainer" is NOT a member of the centre_membership_role enum, so the option cannot be persisted at all. Role lives only on centre_memberships. DO NOT BUILD the dropdown.**
