...
|---|---|
| Screen ID | `23` |
| Screen name | Management Trainers |
| Role | Management |
| Folder | `23-management-trainers` |
| Canonical route | `/management/trainers` |
...
| Figma node | `544:9` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=544-9&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Trainers/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Trainers/Management - Trainers.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Trainers/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Trainers/`** — the ratified Final MVP visual source for this screen. Frame `Management - Trainers.png`, HTML render `Management - Trainers.html`, pack notes `Management - Trainers.md`.
...
Management list over the centre's trainers.
...
Route, layout and components to node `544:9` once a trainer-list projection exists.
...
Do not display or imply a role stored on an identity row. Do not expose authentication details.
...
**Missing** â€” no trainer-list projection.

### Governance

`Post-48-hour final-MVP scope` (A-044). `centre_memberships` is the sole authority for role; a role is never carried on the identity row (A-020, A-025).
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Trainers/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
