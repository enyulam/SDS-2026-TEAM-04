...
|---|---|
| Screen ID | `22` |
| Screen name | Management Edit Student |
| Role | Management |
| Folder | `22-management-edit-student` |
| Canonical route | `/management/students/[studentId]/edit` |
...
| Figma node | `545:175` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=545-175&m=dev |
...
| Screenshot status | ✅ **CURRENT VISUAL REFERENCE EXISTS** — `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Student/`. **Pack-local `reference.png`: not duplicated locally** (optional integrity copy only). ~~Deferred - required for final-MVP completion after physical test~~ — *corrected 2026-08-08: that wording implied no current reference existed, which was false.* |
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Student/Management - Edit Student.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
| Export rule | **No export is required — the ratified frame already exists at `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Student/`.** If a future re-export is ever authorized, it must be the exact node-specific Figma frame; an overview-canvas screenshot is never acceptable. |
...
1. **THE CURRENT `/reference/` PACK — `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Student/`** — the ratified Final MVP visual source for this screen. Frame `Management - Edit Student.png`, HTML render `Management - Edit Student.html`, pack notes `Management - Edit Student.md`.
...
Management edits an existing student record.
...
Route, layout and components to node `545:175` once a student-edit write path and the field inventory exist.
...
Do not invent a student field. Do not add an account linkage to a student (A-025).
...
**Missing** â€” no student-edit write path.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Management - Edit Student/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
