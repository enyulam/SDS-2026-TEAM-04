...
|---|---|
| Screen ID | `02` |
| Screen name | Trainer My Classes |
| Role | Trainer |
| Folder | `02-trainer-my-classes` |
| Canonical route | `/trainer/my-classes` |
...
| Figma node | `777:2` |
| Exact node-specific URL | https://www.figma.com/design/sSY1TYw3jyVlZDy8V2Mu7g/SDS-dashboard?node-id=777-2&m=dev |
...
| Native dimensions | Read them from the current reference frame `UI_REFERENCE_FINAL_MVP/reference/Trainer -  My Classes/Trainer -  My Classes.png`. ~~Not yet known - record … when the screenshot is exported~~ — *corrected 2026-08-08: no export is pending.* |
...
Trainer view over the Class Modules the trainer is assigned to. Under the ratified hierarchy the persisted entity is the Class Module (A-016, A-047).
...
Nothing exists to preserve. Server-side trainer authorization applies once built.

### Allowed frontend expansion

Route, layout and components to node `777:2` once the trainer-scoped class-module projection exists.
...
Do not introduce a hidden `classes` entity (A-016). Do not read class modules client-side without a governed projection.
...
**Missing** â€” no class-module read projection scoped to a trainer.
...
- [ ] The current visual reference `UI_REFERENCE_FINAL_MVP/reference/Trainer -  My Classes/` is the exact node-specific frame, not an overview-canvas capture. **Do NOT gate acceptance on a pack-local `reference.png` — this pack deliberately holds none, and its absence is not a gap.**
