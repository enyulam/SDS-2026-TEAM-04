...
| **GC-6** | 11, 15, 16, 17, 18, 28 | Per-student competency badges, per-dimension bars, "Overall", "Strongest / Focus area" columns to **Management** | ~~`CLAUDE.md` §6 / A-038 — management never reads raw per-dimension ratings~~ ✅ **GROUND CORRECTED 2026-08-11 (`C-18`); the conflict STANDS.** `D-1` lets Management **view** the nine, **read only**, so `A-038` no longer carries this. **Two grounds do, either sufficient:** **`C-9`** — `D-1` reaches **report detail surfaces only**, and none of these six is one (*"it invites comparison between children"*); and **`G-2`** — the `"Overall"` / `"Strongest / Focus area"` limbs are **roll-ups, permanently excluded everywhere**. ⛔ **`28` is additionally deferred outright by `C-11`.** ⚠️ **`GC-5` on screen `19` DID partly discharge — see that pack; `GC-6` did not.** |
