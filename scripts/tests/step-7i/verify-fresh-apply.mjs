...
SELECT (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind='r')
  ...
  // Apply all six migration files in order, exactly as a reset would.
  const files = readdirSync(MIG_DIR).filter((f) => f.endsWith('.sql')).sort()
  if (files.length !== 6) fail(`${files.length} migration files found, expected 6`)
  ...
  pass('all six migrations apply cleanly, in order, from a database with no project objects')
...
SELECT (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind='r')
  ...
  const expected = '26|30|12|29|6|22|0|'
  ...
  else pass(`fresh census: 26 tables, 30 functions, 12 enums, 29 policies, 6 migrations, 22 authenticated EXECUTE, RLS everywhere, 8 ordered labels, 1/3/9 seeds`)

  // Equivalence.
  const fresh = await q(SCRATCH, FINGERPRINT)
  const canon = await q(CANONICAL, FINGERPRINT)
  if (fresh === canon) {
    pass('the canonical fixture database is CATALOGUE-IDENTICAL to a fresh application of the six committed migration files')
  } else {
