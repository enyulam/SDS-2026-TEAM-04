...
// The newest migration file in the tree (Run C3-A Phase 1, the
...
const NEWEST_MIG_NAME = '20260807090000_assessment_complete_save_single_entry_point.sql'
...
// leg — a strictly stronger assertion for it, not a weaker one.
{
  ...
  if (all.length !== 11) fail('T-CT-S4', `${all.length} migration files exist, expected 11`)
  ...
  if (failures === before) pass('T-CT-S4', 'eleven migration files; the ten already-applied files are byte-identical to HEAD; the newest one sorts last')
}
