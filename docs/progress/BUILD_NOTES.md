...
_Next permitted action: review, commit and accept the Step 7E0 documentation checkpoint (the seven staged files above). After acceptance, perform the schema-preflight technical ratification — ratify the `public` profile ↔ `auth.users` relationship, audit target representation, report-status storage, deliberate `GRANT` strategy, enum-versus-reference-table decisions (including Class Grade), invitation token/expiry details, and the first-migration table/enum scope — then produce a bounded Step 7E implementation plan. Do not write SQL, create migrations, seed data, Auth users, RLS, an audit chain, or any application code; do not import or export Figma assets; and do not link the hosted project._
