...
- [ ] **Parent view content checks ~~(Amendment 001 A-002 — Phase 1 is text-only)~~ ✅ (`A-002` AMENDED 2026-08-12 — parent evidence access is Part 1):** on a parent's own, correctly-approved report, confirm there is no per-dimension rating grid anywhere on the page (⛔ **`Q-27` extends this to EVERY parent surface and to the DTOs, projections, RPC results and payloads behind them — not merely to what is drawn**), and that drafts, internal notes, raw ratings and AI history are absent. ~~**and** confirm that **no evidence media is reachable by a parent in Phase 1 at all**. The gated parent-evidence access checks (permitted for the linked child's `Submitted`, consented clip via short-TTL signed URL; refused for every prohibited path) belong to the **Phase 2** review checklist, not here.~~ ✅ **THE GATED PARENT-EVIDENCE CHECKS BELONG HERE NOW**, at `P1-5`, and ⚠️ **BOTH DIRECTIONS ARE MANDATORY** (`A-003`, `A-004`): the **permitted** leg for the linked child's **submitted** clip via a short-TTL server-minted signed URL, **and** every prohibited path refused — unauthorized · unrelated child · pre-`submitted` · expired URL · direct storage path · public object. ⛔ **`A-003`'s `unscanned` leg is `NOT APPLICABLE (C-3)`, never `PASS`** — a must-fail leg with nothing to test either fails forever or gets quietly marked green. ⛔ **A permitted leg without its refusals, or refusals without a permitted leg, is the `S-8` shape and satisfies nothing.** **No download control for any role** (`D-5`).
