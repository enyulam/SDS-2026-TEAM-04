...
16. ⚠️ **A DATA-AVAILABILITY OMISSION AND A RULED OMISSION LOOK IDENTICAL ON A RENDERED PAGE.** `19` omitted Lesson because no projection carried it; it omitted Overall Grade and Term because **G-2 and G-4 rule them out permanently**. ▶ **The first ends when the data arrives; the second never does.** When a phase makes data available, **re-read every omission note on that surface** — leaving a discharged reason in place silently reclassifies a permanent refusal as a temporary gap, and the next phase will "complete" it. Two instances found this batch: the `29` Term chip and `19`'s Report Details note.
