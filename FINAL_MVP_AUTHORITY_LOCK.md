...
- Amendments **001–008** (`docs/spec/BEST_Coach_MVP_Specification_v3_Amendment_00{1..8}.md`), all active. *(✓ extended 2026-08-08: **Amendment 008 / `A-057`** ratified under the Phase-0 ruling set — the minimal two-action evidence audit-registry extension. See `FINAL_MVP_PHASE0_OPERATOR_RULINGS.md`.)* ~~Amendments **001–006** (`docs/spec/BEST_Coach_MVP_Specification_v3_Amendment_00{1..6}.md`), all active.~~ **✅ CORRECTED 2026-08-08 (execution plan P0-T04, under bounded Operator instruction G-00a).** **Amendment 007 / `A-056` is ratified** — it supersedes Amendment 005 A-045's three-rank *visual* ladder and nothing else — and it is cited as active elsewhere in this very file (§28) and in `CLAUDE.md` §1, §7.4. This enumeration was stale by exactly one instrument. **No clause-level supersession is changed by this correction; it records currency only.**
...
**Added 2026-08-11 — the PORTAL DECISION SET (`D-1` … `D-5`).** Client-ratified with **iSpeak Academy** and issued as the input to the **PORTAL COMPLETION PLAN**, the Operator ruled: **`D-1`** Management **may view the nine per-dimension ratings, READ ONLY** — amending **`A-038` in the Management direction only**, with editing still prohibited and an assessment-level disagreement still a **return to the trainer**; ⛔ **`Q-27` is untouched and the Parent boundary does not move** · **`D-2`** a **session score is computed** from the nine dimensions (`Beginning` 25% · `Developing` 50% · `Mastering` 75% · `Mastered` 100%, meaned) **solely to drive a progression trend line**, is **never rendered as a number to any role**, is **not stored**, and holds its mapping in **one place**; ⛔ **it is NOT `G-2`'s "Overall Grade" and `G-2` stands** · **`D-3`** **terms are PERMITTED as scheduling structure** (sessions → lessons → terms, scoping the calendar across all three portals), **reversing `G-4`**, while **end-of-term report generation stays deferred** · **`D-4`** **lesson materials are AUTHORIZED** — Management uploads, Trainer downloads, associated with a **specific class session** — and the frame's **KEY FOCUS chips become PERMITTED in a distinct position with a distinct label**, ⚠️ **never occupying, replacing or visually adjoining the governed carried-over previous-session focus line**, which is unchanged and is the entire reason they were blocked · **`D-5`** **per-child video evidence is AUTHORIZED** — Trainer uploads at assessment time, tagged to exactly one session report, **Management views it before Approve & Submit**, **no download affordance for any role including Parent**, visible to Management · authoring Trainer · linked Parent — **reversing `G-8`** and **activating `A-001`**, whose parent projection was *armed and unactivated*. The canonical instrument is **`FINAL_MVP_PORTAL_DECISIONS.md`** at the repository root. ⚠️ **It authorizes no implementation** — no table, enum, column, audit action string, RPC, grant, policy, bucket, route, migration or screen — and **every one of `D-1` … `D-5` still needs its own explicit Operator authorization.** ⛔ **`A-014` is UNCHANGED: the TA / `Assist.` persona stays deferred and `centre_membership_role` is NOT extended.**

**Added 2026-08-09 — the `G-06` GROUNDING RULE SET RULING.** The Operator ratified the OD-4 grounding rule set, crossing the **`P1-T09` Operator gate** and closing the **last** of the two OD-4 Phase-B sub-rulings (the other, the content-hash envelope version, was ruled as **G-05a** on 2026-08-08). The canonical instrument is **`FINAL_MVP_G06_GROUNDING_RULING.md`** at the repository root, which carries `G06-1` … `G06-8` and the ten required proofs **verbatim**. In summary: **`G06-1`** grounding must assert **COVERAGE of all nine canonical dimension codes** — not a count, and **not satisfiable by a duplicate** — with any unknown / impossible / unmapped rating or dimension state **FAILING CLOSED** · **`G06-2`** the polarity-contradiction rule applies to **`strengths`** · **`G06-3`** it is **NOT** inherited into **`overview`**, whose C7 gap is closed by **widening the achievement lexicon**, never by applying the Strengths rule there · **`G06-4`** it is **NOT** applied to **`areas_for_development`** · **`G06-5`** **Remarks is GROUNDED BUT POLARITY-NEUTRAL** (mapped to the packet's **`R-A`**; `R-B` and `R-C` rejected and the reasons recorded) · **`G06-6`** the panel-wide support-framing keyword escape is **removed** in favour of a **sentence-local + dimension-local** boundary with a **narrowed** lexicon · **`G06-7`** rule 3's fail-open becomes a **deterministic grounding failure** · **`G06-8`** the five measured defects `C4`, `C5`, `C6`, `C7`, `C8` are **authorized for correction**. **It authorizes no schema object, enum value, audit action string, RPC, grant or migration, and weakens no privacy, approval, audit or evidence control** — every change it permits moves in the **fail-closed** direction. That instrument also carries a second, clearly separated **HERO-FIRST RESEQUENCING RULING** (process only): non-hero Phase-2 deferral is a **recorded** disposition, Phase-3 baseline work may begin before Phase 2 completes on proven-stable contracts, **evidence media is OUT of the hero slice** with every A-001/A-003/A-004 safeguard intact and the audit registry held at **16**, and browser legs authenticate by **admin-minted session**, which is **never** described as a sign-in proof.

**Added 2026-08-10 — the HERO CHAIN RULING SET (`G-1` … `G-8`).** Resolving eight collisions found while planning completion of the hero chain — the governed two-stage workflow from the trainer starting a class to the parent reading the submitted report — the Operator ruled: **`G-1`** the three **unframed** hero surfaces (Trainer Review & Approve and both wording editors) **stay unframed**, so the chain's completion criterion **SPLITS** — functional completion covers all eleven hero routes, **visual acceptance covers the eight framed screens only**, and the three are recorded **`VISUAL ACCEPTANCE — NOT APPLICABLE (G-1)`**, ⚠️ **a ruled disposition, never a `NOT-RUN` and never a defect** (§28.4) · **`G-2`** any **"Overall Grade" / roll-up rating** is ⛔ **PERMANENTLY EXCLUDED on all four surfaces, not deferred**, on three independent grounds — the roll-up is **unratified** (`CLAUDE.md` §3.6; A-049 keeps it a proposed, term-generation-only default), on a Management surface it is a **derivation from ratings A-038 bars**, and on a Parent surface it is the **caught leak in softened wording** (Q-27) — so **nothing in the hero chain computes an assessment fact** (§11, §14, §15) · **`G-3`** **lesson number and title are AUTHORIZED as schema**, while the frame's **KEY FOCUS chips are ⛔ PROHIBITED** because they are *lesson-plan intent* and conflating them with the **governed carried-over previous-session focus** would silently replace a governed field (§13, §19.1) · **`G-4`** **term is ⛔ NO** — a display label is not worth building the substrate the §8-deferred End-of-Term generator needs · **`G-5`** the assigned **trainer's name is PERMITTED on the Parent projection**: parents know who teaches their child, the datum is already disclosed on `29` and `06`, and **it is not a rating and not derived from one**, so Q-27 does not reach it (§15) · **`G-6`** **room is AUTHORIZED as schema**, carrying **no authorization meaning** (§13, §19.1) · **`G-7`** **`Main:` is AUTHORIZED** as a projection over `accounts.display_name`, while **`Assist.` is ⛔ PROHIBITED** — it would extend `centre_membership_role` and reactivate the **A-014-deferred TA persona** (§13) · **`G-8`** **class video evidence is ⛔ CONFIRMED OUT**, unchanged, and the frame's "500MB" is **never implemented** against G-05's ruled 50 MiB (§8, §8.1). The canonical instrument is **`FINAL_MVP_HERO_CHAIN_RULINGS.md`** at the repository root. ⚠️ **It authorizes no implementation** — no table, enum, column, audit action string, RPC, grant, policy, route or migration — **extends no enum**, holds the **Step 7H registry at 16**, and **every phase of `docs/plan/HERO_CHAIN_COMPLETION_PLAN.md` still needs its own explicit Operator authorization before any code is written.** **Five of the eight rulings are refusals.**
...
**Added 2026-08-08 — the PHASE-0 OPERATOR RULING SET.** Resolving the Plan Phase-0 decision packet, the Operator ruled on **D-0** (residual synthetic report lifecycle: *preserve first, then governed fixture reload*; classified **synthetic development/verification residue**, and expressly **not** a reason to carry legacy complexity into the OD-4 design) · **G-05a** (the OD-4 content-hash disposition: **V1 frozen byte/semantically unchanged, PARALLEL V2 serializers, new versions at `content_hash_version = 2`, constraint widened to `1 or 2`, no backmigration, no silent relabelling of any future production V1 row, `report_store_draft` keeps ZERO client `EXECUTE`**) · **G-04** (`report_source_map` **REQUIRED**, owner **P2-T06** · `session_logs` **DEFERRED** · the attendance Present/Absent control is a **`GOVERNED FUNCTIONAL INSERTION`** on the existing Trainer roster surface, never a new screen, frame, pack or invented Figma node · **notifications OUT of Final MVP**) · **G-05** (seven evidence dispositions: **no fake scan state**, **no automated retention subsystem**, **50 MiB** ceiling, Management evidence review **read-only with A-038 intact**, **no `consent_records`**, a **two-action** audit-registry extension, and the three §8.2 media classes **deferred**) · **G-03** (seven external inputs recorded `UNKNOWN_EXTERNAL_INPUT` / `NON_BLOCKING_FOR_PLAN_PHASES_0_TO_4`) · **G-02** (usability recruitment **preparation and scheduling authorized**; **adults only, no children, no real child records**; conducting or claiming Phase-9 testing remains a separate gate) · and **no descoping** of the governed scope on calendar pressure. The canonical instrument is **`FINAL_MVP_PHASE0_OPERATOR_RULINGS.md`** at the repository root. ⚠️ ~~**One consequence is recorded there and is not closed:** the authorized fixture reload was proved **structurally impossible** while any audit event exists — `audit_events`' durable actor FK pins `centre_memberships`, and the append-only trigger refuses `DELETE` even for the object-owning `postgres` role — which on Supabase is `rolsuper = false`, `BYPASSRLS` (Lock §18.4), so this is a **trigger** guarantee, not a privilege one — so D-0B is **BLOCKED pending a new Operator decision**, and Plan Phase 1 has **not** started.~~ **✅ RESOLVED 2026-08-09.** The Operator ruled on **B-P0-2** in *"RESOLVE B-P0-2 BY RECREATING THE LOCAL SUPABASE BASELINE"*, authorizing a bounded, **LOCAL-only** destructive recovery: the local Supabase database/stack was recreated from a fresh baseline via the plan's ratified **R-1** semantics (`supabase stop --no-backup` → `supabase start`), **never `supabase db reset`**, after a seven-item pre-destruction gate confirmed the D-0A preservation exists **SHA-identical in both archives**. All 12 committed migrations re-applied from scratch in canonical order; the re-derived census is **unchanged** (12 · 26 · 12 · 34 · 29), and **`report_versions` = 0** with every report-family and audit table at 0. **D-0's disposition is `PRESERVED_THEN_FRESH_LOCAL_RECONSTRUCTION`.** ⚠️ **`B-P0-1`/`B-P0-2` are `RESOLVED_PENDING_FIXTURE_RELOAD`, not yet `CLOSED`** — the governed fixture reload needs the Operator's three interactive no-echo passwords, and the four formerly-red suites clone the canonical fixture database, so Phase 0 has **not** yet exited and Plan Phase 1 has **not** started. *(Preserved per annotate-never-delete: the struck text was accurate on 2026-08-08 and is the evidence that justified the remedy.)*
...
### 2.4 The two authority ladders — A-045 as amended by A-056, and they are NOT one ladder

Amendment 005 A-045 (`…Amendment_005.md:44`, `:134-152`) ratifies **two distinct ladders**. Collapsing them is a governance error. ✅ **Its VISUAL ordering is superseded by A-056 (Amendment 007, 2026-08-08); its two-ladder separation and everything else in it are preserved.**
...
~~**VISUAL authority** (highest first): 1. frozen `reference.png` → 2. node-specific Figma context → 3. existing frontend implementation.~~ ✅ **SUPERSEDED BY RATIFIED AMENDMENT — `A-056` (Amendment 007, 2026-08-08), `docs/spec/BEST_Coach_MVP_Specification_v3_Amendment_007.md`.** That original A-045 wording predates the promotion of `reference/` to visual rank 1 by operator ruling PA-OD-5/5b (§28.1); left as written it ranks a file that **24 of the 36 governed packs do not have** above the ratified frame they all do have.

⚠️ **The instrument changed on 2026-08-08 — this matters for how a future agent cites it.** ~~This lock — not the amendment text — is the instrument that records the reconciliation.~~ **The reconciliation now rests on a ratified amendment (A-056), not on this lock alone.** `docs/spec/…Amendment_005.md` §A-045 remains **unedited and still carries the original ladder** — that is deliberate; ratified specification text is never rewritten in place. **A-056 supersedes ONLY that three-rank ordering.** Everything else in A-045 — the two-ladder separation itself, its functional ladder, *Figma-never-bypasses-governance*, the frame-versus-rule rule, and its preservation of A-022.2 and A-013 — **is expressly preserved by A-056, stands unchanged, and is restated below. A-045 is not obsolete; citing it as active authority remains correct for everything except the visual ordering.**
...
**Rulings for Phase B:** these are **separate media classes requiring separate buckets and separate policies** — do not fold them into the evidence bucket. `Management - Add Trainer.md:21` supplies a **third** uncanonical format/size source (*"supported image formats and maximum file size"*), ~~compounding the `500MB`/`50MiB` conflict in §21~~ ✅ **— that conflict is CLOSED by `C-16` at 100 MiB (corrected 2026-08-12, bounded Operator instruction). ⚠️ THE POINT OF THIS SENTENCE SURVIVES INTACT: none of the three frame-derived format/size sources is ratified authority, and A-022's "do not schema a field from a frame" is unaffected.** The size question stopped being open; **the rule about where size comes from did not change** — And per A-022, **do not schema a field from a frame**: screens 20, 21, 24 and 26 carry unratified field inventories (gender, home address, photo, employee ID) that must not be built from the frame alone.

**Preconditions that are rulings, not implementation details:** `scan_status` (A-001 gate 5) has **no ratified vocabulary anywhere**; the `evidence_media` consent scope (gate 2) has **no table** (§20); and the frame's `MP4, MOV · up to 500MB each` ~~conflicts with `config.toml`'s `file_size_limit = "50MiB"`~~ ✅ **— CONFLICT RESOLVED BY `C-16` AT 100 MiB (`104857600`), corrected 2026-08-12 under a bounded Operator instruction. `config.toml`'s global now reads `100MiB`, and the durable ceiling is the `evidence` BUCKET ROW, not `config.toml`. The frame's `500MB` was never adopted.** ⚠️ **The other two preconditions in this sentence are UNCHANGED and still open:** `scan_status` has no ratified vocabulary — though **`C-3` removed the separate scan gate**, so A-001 gate 5 is `NOT APPLICABLE` rather than pending — and the `evidence_media` consent scope has **no table**, by `C-2`'s centre-level consent ruling and `G-05`'s **no `consent_records`**. Adding an evidence audit action requires an amendment — extending the Step 7H registry is a standing stop-and-ask.
...
**✅ DISCHARGED 2026-08-08 (execution plan P0-T04, under bounded Operator instruction G-00a). THE WARNING ABOVE IS SPENT AND MUST NOT BE ACTED ON.** The reconciliation this paragraph deferred to "Phase B" was in fact performed earlier, by **Phase A2 correction S-06**: the `CLAUDE.md` §5 "Implementation status" bullet now carries the struck original plus *"✅ CORRECTED 2026-08-08 (Phase A2, S-06). V2 AND V3 HAVE BOTH LANDED"*, and the parallel §11 fixture bullet was discharged by **S-07**. **There is no longer any live `CLAUDE.md` sentence instructing an agent not to correct the vocabulary mismatch**, so this section no longer needs to be "authoritative over" it — the two now agree. Verified against the live catalogue at P0-T02 on 2026-08-08: `competency_rating` carries `beginning · developing · mastering · mastered`. *(Line numbers in the struck text are pre-S-06 and no longer resolve; that is expected and is why the struck text is preserved rather than renumbered.)*
...
⛔ **NO ROLL-UP RATING EXISTS — ADDED 2026-08-10 BY OPERATOR RULING `G-2`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §2). **There is no ratified per-session "Overall Grade", and none may be built or derived on any surface.** The **9→7 dimension roll-up** and the **4-level→3-level scale mapping** remain what `CLAUDE.md` §3.6 calls them — **pending client ratification**; **A-049 restated the mapping but expressly preserved its status** as a *proposed, trainer-overridable default used only when term-report generation is built*, and that generator is **out of MVP scope** (`CLAUDE.md` §8). ⚠️ **This is a PERMANENT EXCLUSION, not a deferral**, and it holds independently of the two audience-specific grounds recorded at §14 and §15 — either of which would exclude it on its own.

✅ **`D-2` — A COMPUTED SESSION SCORE IS AUTHORIZED, AND IT IS NOT THE THING EXCLUDED ABOVE. Added 2026-08-11 by operator ruling `D-2`** (`FINAL_MVP_PORTAL_DECISIONS.md`; §2.3). A numeric score is computed from the nine dimensions — **`Beginning` 25% · `Developing` 50% · `Mastering` 75% · `Mastered` 100%**, session score = the mean of the nine, cross-session score = the mean of session scores — **solely to drive a progression trend line.**

⛔ **THE TWO ARE DIFFERENT THINGS AND THE DISTINCTION IS BINDING:**

| | `G-2` "Overall Grade" — ⛔ **excluded** | `D-2` computed score — ✅ **authorized** |
|---|---|---|
| **Rendered?** | as a grade, band or headline rating on a report surface | ⛔ **NEVER rendered as a number, to any role.** It feeds a trend line and nothing else |
| **Stored?** | would be an asserted assessment fact | **Not stored.** Computed from the ratings, mapping in **ONE place** |
| **Status** | **permanently excluded**, all four surfaces | authorized for the trend line **only** |

▶ **If a surface ever displays `D-2`'s value as a number, a band or a grade, that is `G-2` and it is prohibited.**

⚠️ **AND `D-2` RATIFIES ONE MAPPING, NOT THREE.** The client has ratified the **band→percentage** mapping **for `D-2`'s computed, never-rendered value only**. ⛔ **The 4-level→3-level TERM mapping** (`Mastered`→Excellent, `Mastering`→Good, `Developing`/`Beginning`→Needs Improvement) **and the 9→7 dimension roll-up remain PENDING CLIENT RATIFICATION**, exactly as the paragraph above states, and are **a different mapping for a different instrument** — the §8-deferred End-of-Term generator. **Nothing in `D-2` ratifies either of them, and no reader may take one as ratifying the others.**
...
**Added 2026-08-10 by operator rulings `G-3`, `G-6` and `G-7`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §3, §6, §7):

- ⛔ **THE FRAME'S "KEY FOCUS" CHIPS ARE PROHIBITED, AND NO COLUMN IS ADDED FOR THEM.** They are **lesson-plan intent** — what a lesson is *designed* to work on. The roster already renders the **governed carried-over previous-session focus**, a different field with different authority, derived from the trainer's own `follow_up_notes` on the previous session. ⚠️ **They occupy the same visual position, so conflating them would SILENTLY REPLACE A GOVERNED FIELD WITH AN UNGOVERNED ONE — and the substitution would be invisible on the rendered page.** **No phase may render lesson-plan focus into the roster's carried-over focus line, or into any surface presenting the governed focus.** This protects `CLAUDE.md` §10 Phase 1 exit condition **(c)**. The frame's **SLIDES** chips and **View lesson plan** control are prohibited on the same ruling. 

  ✅ **QUALIFIED 2026-08-11 BY OPERATOR RULING `D-4`** (`FINAL_MVP_PORTAL_DECISIONS.md`; Authority Lock §2.3). **KEY FOCUS chips are now PERMITTED — in a DISTINCT VISUAL POSITION with a DISTINCT LABEL, and nowhere else.** Their purpose is to give a trainer a quick refresher on what the session covers, before class, alongside the `D-4` lesson-materials feature. ⛔ **THE PROHIBITION THAT MATTERED IS UNCHANGED AND STILL ABSOLUTE: they must NEVER occupy, replace, or visually adjoin the governed carried-over previous-session focus line, or any surface presenting the governed focus.** ⚠️ **That was always the real ruling.** The chips were never dangerous as content — they were dangerous *in that position*, because two different things in one place is an **invisible substitution of a governed field**, and the substitution cannot be seen on the rendered page. **This still protects `CLAUDE.md` §10 Phase 1 exit condition (c).** ▶ **Permitted elsewhere; prohibited there. The position is the rule.** *(`D-4` says nothing about the frame's **SLIDES** chips or **View lesson plan** control beyond the lesson-materials feature it authorizes; both remain governed by `G-3` and by `D-4`'s own scope.)*
- ✅ **Lesson number, lesson title and room are authorized as session-descriptive schema** (§19.1). ⚠️ **`room` carries NO authorization meaning and must never scope a query** — trainer reach is proved through the live class-session assignment (ADR-4), never through a location.
- ✅ **`Main:` — the assigned trainer's display name — is authorized**, and needs **no schema**: it is reachable today via `class_session_assignments.trainer_membership_id → centre_memberships.account_id → accounts.display_name`. ⛔ **`Assist.` is PROHIBITED.** A second staff role is not a label — `class_session_assignments.trainer_role` is typed `centre_membership_role`, so it would **extend a governed enum carrying authorization vocabulary** and **reactivate the A-014-deferred Teaching Assistant persona**. **`centre_membership_role` is NOT extended.**

⚠️ **`G-6` and `G-7` DISCHARGE AN ALREADY-REGISTERED DEPENDENCY rather than open new scope.** Screen `05`'s pack records at checkpoint **F-04** that *"the frame's session room/location ('Studio 2') and its Main / Assist. trainer names exist on no governed field and are **omitted rather than fabricated**"*. The gap was correctly **recorded**, not invented around; these rulings close it.
...
✅ **BUT MANAGEMENT MAY NOW *VIEW* THE NINE PER-DIMENSION RATINGS — ADDED 2026-08-11 BY OPERATOR RULING `D-1`** (`FINAL_MVP_PORTAL_DECISIONS.md`; §2.3). **READ ONLY.** The `CANNOT modify` line above is **unchanged and still absolute** — ⚠️ **`D-1` separates SEEING from CHANGING, and that separation is the whole ruling.** Management is the highest authority in the academy and approves reports about children, so oversight of the underlying assessment is reasonable; **assessment remains the trainer's job**, and an assessment-level disagreement is still a **return to the trainer**, never a management edit. ⛔ **`Q-27` is untouched — `D-1` moves A-038 in the MANAGEMENT direction only and grants Parent nothing.**

⛔ **AND MANAGEMENT CANNOT RECEIVE A ROLL-UP RATING — ADDED 2026-08-10 BY OPERATOR RULING `G-2`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §2). The frames draw an **"Overall Grade"** on `19`; it is **never built**. ~~⚠️ **A roll-up is computed FROM the nine ratings, so permitting it would deliver their aggregate to a reader A-038 bars from their substance** — the bar is on the assessment data, not merely on its presentation format.~~ ⛔ **THAT REASON LAPSED ON 2026-08-11 — operator ruling `D-1`** (`FINAL_MVP_PORTAL_DECISIONS.md`) makes the nine ratings **read-only visible to management**, so there is no longer a bar for an aggregate to smuggle past. Struck and preserved per annotate-never-delete. ✅ **THE EXCLUSION ITSELF IS UNCHANGED AND STILL PERMANENT.** `G-2` rested on **three independent grounds** and `D-1` removed exactly one; either survivor excludes it alone: **(1)** the roll-up is **unratified** — §11, `CLAUDE.md` §3.6, and A-049 keeps the 4→3 mapping a *proposed, term-generation-only* default; **(2)** on a **Parent** surface it is the **caught leak in softened wording** (`Q-27`, §15). ⚠️ **Recorded rather than quietly dropped, because a rule whose stated reason has gone stale is exactly what invites a later phase to "fix" it.** **This is a permanent exclusion, not a deferral.** ⛔ **And `D-2`'s computed session score is NOT this roll-up** — see §11.
...
- ✅ **The assigned trainer's display name IS permitted — ADDED 2026-08-10 by operator ruling `G-5`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §5). **Reasoning:** parents know who teaches their child and withholding it is meaningless secrecy; the same datum is already disclosed on `29` (A-019) and `06`; and ⚠️ **it is not a rating and not derived from one, so Q-27 does not reach it** — which is precisely why it required its own ruling rather than an inference either way. **This widens the Parent projection by exactly one field and nothing else.**
- ⛔ **Nothing may disclose that a correction cycle is or was underway** — carried from the `32` and `33` pack records and binding on every Parent surface, alongside the no-revision-number and no-correction-reason rules above.
...
**Implementation status — this ruling authorizes nothing.** The database, RPCs, types, prompt, grounding, fixtures, UI and tests all still carry the superseded names. That is a **registered Phase B migration** (`FINAL_MVP_OD4_REPORT_SEMANTICS_RULING.md` §4–§5), not a contradiction of this section. ~~Two items there are **genuine open Phase B rulings**: the content-hash envelope version (the panel names are literal bytes inside both SHA-256 preimages, and `content_hash_version` is pinned by `CHECK (= 1)`), and~~ **✅ CORRECTED 2026-08-08 — ONE remains open, not two.** The **content-hash envelope version is RULED** (G-05a, 2026-08-08: **V1 frozen, PARALLEL V2, new versions at `content_hash_version = 2`, CHECK widened to `1 or 2`, no backmigration** — `FINAL_MVP_PHASE0_OPERATOR_RULINGS.md`). ~~**The one genuine open Phase B ruling that remains is** the re-derivation of grounding rule 4, which is hard-keyed to the old single-positive-panel model and would **false-reject** correctly-grounded drafts if simply retargeted at `overview`.~~ **✅ CORRECTED 2026-08-09 — ZERO open OD-4 Phase B sub-rulings remain.** The grounding rule-4 re-derivation was **RULED as `G-06`** on 2026-08-09; the canonical instrument is **`FINAL_MVP_G06_GROUNDING_RULING.md`** (repository root, §2.3 class — indexed at §2.3 above). It was **re-derived, never renamed**: rule 4 is scoped to `strengths`; `overview` and `areas_for_development` do **not** inherit it (so the false-rejection this line warned about is avoided by design, not by accident); **Remarks is grounded but polarity-neutral** (`R-A`); a **new rule 1b** asserts nine-code COVERAGE and fails closed on unmapped state; and rule 3's lexicon was widened rather than any panel being given a new polarity posture. **It is IMPLEMENTED and PROVEN** at plan task `P1-T09` (`scripts/tests/g6-harness/prove-g06-grounding.mjs`, run by `run-canonical.mjs`), with every new control demonstrated capable of **firing** against a planted regression. *(Also corrected the same day: the second G-06 decision widened the achievement lexicon to celebratory wording, closing the last measured residual.)* **This line's own count is therefore now spent — the "two → one" correction of 2026-08-08 is superseded by "one → zero".**
...
- **Trainer and Management rating AUTHORITY is unchanged — neither widened nor narrowed.** §9, §11, §13 and §14 are untouched, and that expressly ~~**includes A-038's standing bar on Management reading raw per-dimension assessment data**, and GC-5/GC-6, which remain live conflicts~~ ✅ **CORRECTED 2026-08-11 (`C-18`): that clause was TRUE WHEN `Q-27` WAS RULED AND IS NOW STALE.** Operator ruling **`D-1`** (2026-08-11) **moved A-038 in the Management direction**: Management may **VIEW the nine per-dimension ratings, READ ONLY**, on a **report detail surface** (`C-9`), rendering **all nine** rather than the frame's four (`C-10`). **`GC-5` is therefore partly discharged on screen `19`; `GC-6` is NOT** — screens `11 15 16 17 18 28` are list, statistics and profile surfaces that `C-9` expressly excludes, and their roll-up limbs stay `G-2`. ⛔ **`Q-27` still grants Management nothing, and this correction grants PARENT nothing whatsoever** — it records a change to the Management boundary that happened elsewhere, three days after `Q-27`. **Management may VIEW, never EDIT: an assessment-level disagreement is a return to the trainer.** Read precisely: the Trainer continues to author the nine ratings under §13; Management's authority over them remains exactly what A-034/A-038 already allow and no more. This ruling concerns **Parent visibility only** — it is **not** a general prohibition on ratings, and it must not be quoted in either direction as a change to the Trainer or Management boundary.
...
**Added 2026-08-10 by operator rulings `G-3` and `G-6`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §3, §6): **three session-descriptive columns on `class_sessions` — `lesson_number`, `lesson_title` and `room` — are AUTHORIZED IN SCOPE.** They add **no table, no enum and no policy**, so **the census figures above are unchanged today**; on implementation the **migration count moves by one** and nothing else in the census does.

⚠️ **A SCOPE RULING IS NOT A MIGRATION AUTHORIZATION.** `G-3`/`G-6` settle **whether** these columns belong in the Final MVP, not **that a migration may be written**. Writing it remains a separate, explicitly-named Operator authorization (`CLAUDE.md` §12), and A-032's bar on placeholder or dangling columns is untouched.
...
- `allowed_mime_types` and `file_size_limit` must be set from ratified authority — **not** from the reference pack's HTML, which is design intent rather than authority. ~~The `500MB` / `50MiB` conflict must be resolved by ruling before implementation.~~ ✅ **RESOLVED AND IMPLEMENTED — corrected 2026-08-12 under a bounded Operator instruction.** `C-16` ruled **100 MiB (`104857600`)**; the shipped `evidence` bucket carries `file_size_limit = 104857600` and `allowed_mime_types = ["video/mp4","video/quicktime"]`, both set from the ruling and not from the pack HTML. **The precondition this bullet states was satisfied before implementation, exactly as required.**
...
> ✅ **And since 2026-08-08 the ordering is RATIFIED, not merely propagated: `A-056` (Amendment 007).** The synchronization below aligned the carriers; **A-056 supersedes A-045's three-rank ladder at specification level**, so the model no longer rests on carrier prose contradicting a ratified instrument. **A-045 remains active authority for everything else it says.** `docs/spec/BEST_Coach_MVP_Specification_v3_Amendment_007.md`.
...
**Added 2026-08-10 by operator ruling `G-1`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §1). Three surfaces on the **hero chain** have no frame — **Trainer Review & Approve** plus the **Trainer** and **Management wording editors**, the latter two being U-25 families from the list above. **They stay unframed; commissioning frames is a design task and is not on the hero-chain path.** They remain on their siblings' foundation — the same tokens, primitives and shell.

**Consequence — the hero chain's completion criterion SPLITS:** functional completion covers all **eleven** hero routes; **visual acceptance covers the eight framed screens only**, and the three are recorded **`VISUAL ACCEPTANCE — NOT APPLICABLE (G-1)`**.

⚠️ **`NOT APPLICABLE (G-1)` is a RULED DISPOSITION — never a `NOT-RUN`, and never a defect.** It must not be reported as a pass, and must not be reported as a gap. ⚠️ **This ruling closes ZERO U-25 families and permits no invented frame, node ID or field** — it rules on **acceptance**, not on design existence, and the prohibition in the paragraph above is untouched.
...
- This Authority Lock
- spec v3 · Amendments 001–008 — **functional rank 1** ~~spec v3 · Amendments 001–006 — **functional rank 1**~~ *(✅ corrected 2026-08-08, P0-T04 under G-00a: Amendment 007 / `A-056` is ratified and active. ✅ extended again 2026-08-08 under the Phase-0 ruling set: **Amendment 008 / `A-057`** is ratified and active. Rank unchanged by either.)*
...
2a. **⚠️ `SDS Project Final (BEST Coach)/docs/spec/` in full — spec v3 and Amendments 001–008 — and `CLAUDE.md`.** ~~spec v3 and Amendments 001–006~~ *(✅ corrected 2026-08-08, P0-T04 under G-00a: Amendment 007 / `A-056` is ratified; ✅ extended again 2026-08-08 under the Phase-0 ruling set for **Amendment 008 / `A-057`**. **The protection was never narrower than the directory** — this clause already protects `docs/spec/` "in full", so Amendment 007 was protected before this correction; only the enumeration was stale.)* These are **functional ladder ranks 1 and 2**, the top two rungs of the authority this entire lock rests on. *(An earlier draft of §31 omitted both, protecting the migrations and the screen packs while leaving the governing specification unlisted. That was the most consequential gap in the protection list.)* Also protected: `docs/plan/` and `docs/progress/` in full.
...
~~⚠️ **One exception, added after review: `OD-4` (§15.1) IS a genuine operator decision, and it is time-boxed.** It does not block this lock — the governed panel names are already implemented and are what §15 ratifies — but it becomes **irreversible once `report_versions` holds a row**, and it is the one item here that a later ruling cannot recover.~~ ✅ **RULED 2026-08-07 — `OD-4` IS CLOSED.** The operator ratified *Overview · Strengths · Areas for Development · Remarks* as the Final MVP report semantics, inside the time-box, while `report_versions` was still empty. See §15.1 and `FINAL_MVP_OD4_REPORT_SEMANTICS_RULING.md`. **What remains is a registered Phase B implementation migration, not an open decision** — ~~with two genuine Phase B sub-rulings inside it (the content-hash envelope version; the grounding rule-4 re-derivation).~~ **✅ CORRECTED 2026-08-09 — BOTH sub-rulings are now CLOSED, so ZERO remain.** The **content-hash envelope version** was ruled as **`G-05a`** on 2026-08-08 (`FINAL_MVP_PHASE0_OPERATOR_RULINGS.md`), and the **grounding rule-4 re-derivation** was ruled as **`G-06`** on 2026-08-09 (`FINAL_MVP_G06_GROUNDING_RULING.md`, repository root, indexed at §2.3). G-06 is additionally **implemented and proven** at plan task `P1-T09`. Everything else below is an operator-held fact, a human action, or a Phase B implementation choice.
...
| ~~`500MB` vs `50MiB` evidence size limit (§21)~~ ✅ **CLOSED — RULED, NOT OPEN. `C-16` = 100 MiB (`104857600`), 2026-08-11.** ⛔ **This row presented a SETTLED question as an OPEN register row, which invites a session to re-decide what was already ruled** — the most dangerous of the ten stale sites, corrected 2026-08-12 under a bounded Operator instruction. **`G-05`'s 50 MiB was superseded** (premise lapsed: chosen without video in mind); **the frame's `500MB` was NEVER adopted** — `C-16` raised the limit *"but NOT to 500 MB"*. **Enforced in shipped schema in three independent places** by `20260812090000_portal_d5_evidence_substrate.sql`, incl. assertion `E4`, which fails the migration if the bucket limit differs | ~~Phase B ruling~~ **RULED — `C-16`** | ~~Medium~~ **CLOSED** |
