...
⛔ **AND MANAGEMENT CANNOT RECEIVE A ROLL-UP RATING — ADDED 2026-08-10 BY OPERATOR RULING `G-2`** (`FINAL_MVP_HERO_CHAIN_RULINGS.md` §2). The frames draw an **"Overall Grade"** on `19`; it is **never built**. ⚠️ **A roll-up is computed FROM the nine ratings, so permitting it would deliver their aggregate to a reader A-038 bars from their substance** — the bar is on the assessment data, not merely on its presentation format. **This is a permanent exclusion, not a deferral**, and it stands independently of the unratified-roll-up ground at §11.
