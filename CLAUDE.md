...
**This subsection is authoritative over any older spec §20 table name or shape.** It records *what the first governed migration would contain* — it is **not** permission to write it.

> **Step 7E is blocked, unauthorized and unstarted.** Amendment 003 governs schema architecture and migration boundaries; **it does not authorize migration implementation.** Do not write SQL, create a migration file, seed data, or touch the database on the strength of this section. **Step 7E requires its own explicit orchestrator authorization.**
...
> **Step 7E is blocked, unauthorized and unstarted, and stays that way until the Step 7E0D ratification checkpoint is accepted and committed *and* the orchestrator issues an explicit Step 7E authorization.** Amendment 003 fixes the migration's contents; it does not authorize the migration.
